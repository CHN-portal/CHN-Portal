...
export BUILDER_IMAGE_NAME="cbdhealthnetwork-builder-$BUILD_TARGET-$JOB_NUMBER"
...
if [ "$BUILD_TARGET" = "arm-linux" ]; then
  ...
  export USE_SHELL="/bin/cbdhealthnetwork"
  export PYZMQ=true
  export RUN_TESTS=true
  ...
  export CPPFLAGS="-DDEBUG_LOCKORDER -DENABLE_CHN_DEBUG"
  export PYZMQ=true
  export RUN_TESTS=true
