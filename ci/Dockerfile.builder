...
# cbdhealthnetwork_hash
RUN git clone https://github.com/cbdhealthnetworkpay/cbdhealthnetwork_hash
RUN cd cbdhealthnetwork_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} cbdhealthnetwork
RUN useradd -u ${USER_ID} -g cbdhealthnetwork -s /bin/bash -m -d /cbdhealthnetwork cbdhealthnetwork
...
RUN mkdir /cbdhealthnetwork-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /cbdhealthnetwork-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /cbdhealthnetwork-src

USER cbdhealthnetwork
