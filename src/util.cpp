...
//CbdHealthNetwork only features
...
const char * const BITCOIN_CONF_FILENAME = "cbdhealthnetwork.conf";
const char * const BITCOIN_PID_FILENAME = "cbdhealthnetworkd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "cbdhealthnetwork" is a composite category enabling all CbdHealthNetwork-related debug output
                if(ptrCategory->count(std::string("cbdhealthnetwork"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CbdHealthNetwork
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CbdHealthNetwork
    // Mac: ~/Library/Application Support/CbdHealthNetwork
    // Unix: ~/.cbdhealthnetwork
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CbdHealthNetwork";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CbdHealthNetwork";
#else
    // Unix
    return pathRet / ".cbdhealthnetwork";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty cbdhealthnetwork.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override cbdhealthnetwork.conf
