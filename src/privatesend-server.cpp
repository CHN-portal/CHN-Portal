...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all CbdHealthNetwork related functionality
...
/*
    ...
    allow endless transaction that would bloat CbdHealthNetwork and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all CbdHealthNetwork specific functionality
