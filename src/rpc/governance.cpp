...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 CHN irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on cbdhealthnetworkcentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in cbdhealthnetwork.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("cbdhealthnetwork.conf", statusObj));
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by masternode configured in cbdhealthnetwork.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    ...
    { "cbdhealthnetwork",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "cbdhealthnetwork",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "cbdhealthnetwork",               "gobject",                &gobject,                true,  {} },
    { "cbdhealthnetwork",               "voteraw",                &voteraw,                true,  {} },
